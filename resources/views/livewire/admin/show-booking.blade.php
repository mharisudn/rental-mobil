        ...
        <div class="bg-white shadow-lg rounded-sm border border-slate-200">
            <header class="block justify-between items-center sm:flex py-6 px-4">
                <h2 class="font-semibold text-slate-800">
                    All Booking
                    <span class="text-slate-400 font-medium">{{ $item->total() }}</span>
                </h2>
                    ...
                    @if(!$selected)
                        ...
                        <span class="absolute inset-y-0 left-0 flex items-center px-2 cursor-pointer">
                            ...
                            <input wire:model.live.debounce.500ms="search"
                                   ...
                                   item="text" placeholder="Search by name" autocomplete="new-text">
                        </div>
            ...
            <div class="overflow-x-auto">
                <table class="table-auto w-full">
                    ...
                    <tr>
                        ...
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                            <div class="font-semibold text-left">Booking Status</div>
                        </th>
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    ...
                    @forelse($item as $key => $item)
                        <tr wire:key="row{{ $item->id }}">
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->brand->name ?? '-' }}</td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->type->name ?? '-' }}</td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->start_date }}</td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->end_date }}</td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->status }}</td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->payment_status }}</td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">{{ $item->amount }}</td>
                                        ...
                                        <ul>
                                            <li class="cursor-pointer hover:bg-slate-100">
                                                <div class="flex items-center space-x-2 p-3">
                                                    ...
                                                    <span>
                                                        <a href="{{ route('admin.booking.edit', $item->id) }}">Edit</a>
                                                    </span>
                                                </div>
                                            </li>
                                            <li class="cursor-pointer hover:bg-slate-100">
